...
def batch_classify():
    """批量分类"""
    # 加载未分类文件
    with open('unclassified_prompts.json', 'r', encoding='utf-8') as f:
        unclassified = json.load(f)

    classifier = AIDomainnClassifier()
    results = []

    print(f"开始AI分类 {len(unclassified)} 个文件...\n")
    ...
    # 保存结果
    with open('ai_classification_results.json', 'w', encoding='utf-8') as f:
    ...
    print(f"\n结果已保存到: ai_classification_results.json")
